...
ClassImp(o2::endcaps::TopologyDictionary);
...
math_utils::Point3D<float> TopologyDictionary::getClusterCoordinates(const o2::itsmft::CompCluster& cl) const
{
  math_utils::Point3D<float> locCl;
  o2::endcaps::SegmentationAlpide::detectorToLocalUnchecked(cl.getRow(), cl.getCol(), locCl);
  locCl.SetX(locCl.X() + this->getXCOG(cl.getPatternID()));
  locCl.SetZ(locCl.Z() + this->getZCOG(cl.getPatternID()));
  return locCl;
}

math_utils::Point3D<float> TopologyDictionary::getClusterCoordinates(const o2::itsmft::CompCluster& cl, const ClusterPattern& patt)
{
  float xCOG = 0, zCOG = 0;
  patt.getCOG(xCOG, zCOG);
  math_utils::Point3D<float> locCl;
  o2::endcaps::SegmentationAlpide::detectorToLocalUnchecked(cl.getRow() - round(xCOG) + xCOG, cl.getCol() - round(zCOG) + zCOG, locCl);
  return locCl;
}
